...
$result=mysqli_query($con,"SELECT * from customerTable")or die('error');
?>
...
<div class="container-fluid"  style="height:0px;margin-top: 3%;">
  <button  style="width:10%;height: 40px;margin-right: 50%;
    ...
    <button class="span" onclick="window.location.href='customer list.php'" style="color: #1097A1;">List</button>
    </div>
     ...
     <table id="example" class="table table-striped table-bordered" style="width:100%;">
      ...
      <?php while($row=mysqli_fetch_array($result)) { ?>
            <tr style="background-color: white;">
              <td><?php echo $row['customerId']?></td>
                ...
                <td><?php echo $row['name']?></td>
                 ...
                 <?php $res=mysqli_query($con,"SELECT COUNT(vehicleId) FROM vehicleDetails where customerId='$cid'")or die('error');
                while($row1=mysqli_fetch_array($res)){; 
                 ?>
                 <td><?php echo count($row1)?></td>
               <?php }?>
               <?php 
               $res1=mysqli_query($con,"SELECT * FROM addressTable where userId='$cid'")or die('error');
                 while($row2=mysqli_fetch_array($res1)) { 
               ?>
                 <td><?php echo $row2['currentAddress']?></td>
               <?php }?>
                 <td>
             <div class="container" style="width: 50%;">
               <a class=btn1 href="customerview.php?edit=<?php echo $row['customerId'];?>"><i class="fa fa-eye" style="font-size: 13px;height: 20px;top: 2px;right: 3px;">View</i></a>
              <a class=btn2 href="customerupdate.php?edit=<?php echo $row['customerId'];?>"><i class="fa fa-pencil-square-o" style="font-size: 13px;height: 24px;top: 3px;">Update</i></a>
              <button type="button" class="btn3" onClick="deleteme(<?php echo $row['customerId']; ?>)" value="Delete"><i class="fa fa-trash-o" style="font-size: 13px;height: 20px;top: 2px;">Delete</i></button>
            </div>
            </td> 
            </tr> 
            ...
            <script language="javascript">
              ...
              window.location.href='server.php?del_customer=' +delid+'';
              return true;
              }
              }
            </script>
