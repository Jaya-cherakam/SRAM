...
$result=mysqli_query($con,"SELECT * FROM customerComplaint WHERE complaintId='$edit'")or die('error');
?>
        ...
        <button type="button" class="close" data-dismiss="modal">&times;</button>
          <form action="server.php" method="post">
            <?php while($row=mysqli_fetch_array($result1)) {   ?> 
              <input type="hidden" name="assignedby" value="<?php echo $row['empId']?>">
            <?php }?>
            <?php while($row3=mysqli_fetch_array($result)) {   ?> 
              <input type="hidden" name="cid" value="<?php echo $row3['complaintId']?>">
            <?php }?>
          <div class="form-group">
           ...
           <div class="form-group">
            ...
            <select class="form-control input-lg" name="serviceboy">
              <?php $res=mysqli_query($con,"SELECT * FROM employeeTable where role!='admin'")or die('error');?>
                                <?php while($row1=mysqli_fetch_array($res)) {   ?> 
                              <option value="<?php echo $row1['empId'] ?>"><?php echo $row1['empId'].'-'.$row1['name']?></option>
                              <?php }?>
                              </select>
          ...
          <div class="form-group">
            ...
            <input type="date" class="form-control" id="Target Time" placeholder="" name="targettime">
            </div>
          <div class="form-group">
            <label for="tos">Take Instruments:</label>
            <input type="text" class="form-control" id="Take Instruments" placeholder="Take instruments" name="instruments">
            </div>
          
        <button id="btnSearch" class="btn btn-primary btn-md center-block" Style="width: 100px;" name="assigntask">Submit</button>
         </form>
        </div>
      </div>
    </div>
