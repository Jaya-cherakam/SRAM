...
$result=mysqli_query($con,"SELECT * FROM updates where id=(SELECT MAX(id) from updates)")or die('error');
?>
...
<span style="font-size: 20px;margin-left: 45%;height:20px;border:1px solid gray;border-radius: 10px;background: white;color: #1097A1;">Updates</span>
</div>
...
<div class="col-sm-6 col-xs-9" style=" margin-left: 25%;margin-top:%;">
  <div class="form-group">
    <?php while($row=mysqli_fetch_array($result)) {
     ?>
      <textarea class="form-control" rows="5" id="comment" placeholder="Comment:"><?php echo $row['updateDescription']?></textarea>
    <?php }?>
  </div>
  </div>
...
#btnSearch{
   ...
   <div class="row">
    ...
    <button id="btnSearch" Style="width: 80px;background-color:#FC1837;"  onclick="window.location.href='selectgroup.php'">Add</button>
   </div></div>  
  </body>
</html>
<?php }else{
